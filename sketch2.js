...
let canvasWidth = 400;    // Largeur de la boîte de message
let canvasHeight = 300;   // Hauteur de la boîte de message
...
function draw() {
    ...
    function keyPressed() {
        if (key === 'e') {
            // Quand la touche 'e' est pressée, afficher des carrés verts sur le deuxième canvas
            canvas2.clear();  // Effacer l'ancien dessin
            canvas2.fill(0, 255, 0);  // Vert
            canvas2.noStroke();
    
            // Dessiner plusieurs carrés verts à différentes positions
            for (let i = 0; i < 5; i++) {
                canvas2.rect(50 + i * 60, 100, 50, 50);  // Dessiner les carrés sur le canvas2
            }
            image(canvas2, 0, 0);  // Afficher le deuxième canvas dans le canvas principal
        }
